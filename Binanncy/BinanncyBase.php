<?php
if(!class_exists("BinanncyBase")) {
	class BinanncyBase {
    	public $key = "6AD39BFCB9884657";
    	private $product_id = "1";
    	private $product_base = "binanncy";
    	private $server_host = "https://btctech.co.uk/wp-json/licensor/";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_binanncy_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=binanncy_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
